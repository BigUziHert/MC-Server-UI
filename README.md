...
The **Server**, **Minecraft**, and **Management** sidebar headings collapse and reopen their sections. Each choice is remembered in this browser. The Databases page has been removed; existing SQLite files and saved records are preserved.
...
- **Backups:** create and download real `.tar.gz` archives; configure interval, daily, or weekly schedules and retention. Select individual archives or **Select all**, then use **Delete selected** to move them to Recycle Bin. Each row also has download and delete buttons. Restoring a backup from Recycle Bin returns the archive to backup history without extracting it or changing server files. Failed moves remain selected for retry. Manual backups are kept until deleted. Retention moves excess scheduled backups to Recycle Bin after a new archive succeeds; recycled archives continue using disk space until explicitly deleted permanently. Symbolic links are excluded.
- **Subusers:** invite people by email to an authenticated panel on their phone or browser. Choose per-server Control, User, Files, Backups, Databases, and Audit permissions, or use the Control preset for console and power controls. Permission changes apply to active sessions immediately; removing a subuser revokes their access to that server. Invitations and sign-in emails use Resend after remote access is configured. Existing role records keep their equivalent permissions but require an invitation before remote access is granted.
- **Audit Logs:** server starts, restarts, stops, icons, API-key settings (without secret values), and explicit EULA acceptance appear under **Server**. File edits, directory deletions, and mod/plugin/datapack additions, updates, restores, and deletions appear under **Files**. **Players** contains operator, ban, kick, and whitelist actions; commands are recorded as requests sent to Minecraft. Backups and Subusers have their own categories. Scheduled actions identify **Scheduler**, while observed exits identify **Server process**; user actions default to **Local administrator**. Installation failures and scheduled backup retention are recorded too. Legacy database activity is hidden. The most recent 2,000 events are retained per server. Panel history keeps setup and server-removal events available after a server is removed. The table refreshes every ten seconds while visible, retains rows during manual refresh, and supports search and pagination.
...
Use **Remove** on an installed mod to review removal while the server is stopped. Launchpad reads local Forge, NeoForge, Fabric, or Quilt declarations, including embedded libraries, and blocks removal when another installed mod requires it or dependency metadata cannot be read. Large mod archives and declared library containers are inspected without extracting their resources. Confirmed dependent mods are listed separately from an incomplete dependency check; unreadable files appear in expandable details with a retry button. Alternate and conditional requirements are treated conservatively. Only the selected JAR moves to Recycle Bin; its dependencies, configuration, and other installed mods remain. Removing or replacing a mod after review invalidates that review. Restore a removed mod from File Manager's Recycle Bin.
...
Text search fields include a small clear button and do not add outlines or highlights when focused. Clearing a search keeps keyboard focus in the field.
...
The local panel remains the owner's administration interface. Remote access uses a separate authenticated listener, disabled until configured in **Subusers → Set up phone access**.

1. Set up an HTTPS reverse proxy or tunnel with a public origin such as `https://panel.example.com`. Route it to `http://127.0.0.1:3002` on the computer running MC Panel. Use a dedicated hostname without a path. Configure the hostname, DNS, and HTTPS with your provider; MC Panel does not create them automatically. Preserve the public `Host` header so it matches the configured panel URL.
2. In Resend, verify a sending domain and create an API key with permission to send email. Use a sending address on that domain, such as `MC Panel <panel@example.com>`. The default test sender cannot deliver invitations to arbitrary recipients.
3. Open **Subusers** in the local owner panel. Enter the **HTTPS panel URL**, **Sending address**, and **Resend API key**. Keep **Remote access port** at `3002`, or choose an unused port and update the proxy target to match. Check **Enable remote access**, then **Save access settings**. A saved key is never returned to the browser; leaving its field blank keeps it.
4. Open the public HTTPS address from the recipient's phone, using mobile data to check access from outside your home network. The sign-in screen should appear. Keep the computer, MC Panel, and the proxy or tunnel running whenever remote access is needed.
5. Select the server, click **New user**, enter the recipient's email, and choose permissions. **Use Control preset** grants console, start, stop, and restart. Leave **Send invitation by email** checked and click **Create subuser**. You can also save a subuser before setup and use **Send invite** later. If delivery fails, the subuser remains saved; retry the invitation from their row.

Invitation links work once and expire after 24 hours. The recipient opens the link and confirms sign-in; their session lasts up to seven days. Later they can request a fresh sign-in email from the public panel; those links work once and expire after 15 minutes. **Invite again** sends another invitation. **Save permissions** changes what the subuser can do immediately. Removing their access disables their sessions and invitation links for that server.

Only servers the signed-in person has been invited to appear in the remote session. Permission checks apply to every server request, including direct downloads. Access settings, server creation/import/removal, and owner configuration remain local administrative actions. A delegated user can only grant permissions they also have. Remote settings and authentication state are stored in `remote-access.json` inside the panel data directory; protect that directory because it contains the email delivery credential.

## Local access boundary

The owner API and development frontend bind to `127.0.0.1`. The owner API rejects nonlocal Host headers and cross-site mutation requests. The desktop app additionally protects its randomly assigned local port with a private session cookie and accepts only its own origin. **Never route a public proxy or tunnel to the owner API (`3001` by default), Vite development server, or desktop session port.** Route only to the separate remote access port (`3002` by default), which requires email sign-in, a secure session cookie, and the subuser's server permissions. HTTPS terminates at the proxy or tunnel; the remote listener uses HTTP behind it.
...
Integration tests exercise original-byte uploads/downloads, editing and deletion, path traversal and symlink rejection, request-origin checks, readable backup archives, persistence and retention, missed schedule handling, SQLite validity, access records, process-backed console controls, and online Java backup coordination with a controlled process fixture. Fleet tests also check server isolation, selector validation, legacy migration, rename persistence, unique-port creation races, launch races, and validated OP/deOP commands. Tests use isolated temporary directories and do not touch your server data.

Browser integration tests cover the console, file editor and transfers, backup downloads and schedules, subuser permissions and invitation retries, SQLite downloads, audit filtering, and mobile layouts:
