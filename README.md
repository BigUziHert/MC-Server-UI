...
A fresh desktop workspace starts with **Welcome to MC Panel**, where you can **Create a new server** or **Import an existing server**. No server is registered until you confirm its setup.
...
Choose **Updates** to update MC Panel on the computer you are using. While connected to another panel, this opens a separate update window and keeps the remote server selected, including when this computer has no local servers.
...
The desktop app switches to the remote panel inside the same window. The **Server selector** keeps local servers under **This computer** and each signed-in remote panel's servers under its address. These groups stay visible when you switch between local and remote servers. Select any server to open its Console while keeping the other connections signed in. Signing out removes that panel's servers from the selector; changes to your server access update the list automatically. The account menu also lets you switch between connected remote panels. The remote sign-in screen provides **Back to this computer**. The browser edition opens connections in the current tab. Accept the host's invitation and choose a password to create your panel login. New panel accounts can access all current and future servers by default; the owner can exclude individual servers or allow only selected ones.

To add servers from another PC, open **Subusers** on the host, edit your account, and enable **Computer permissions → Create and import servers**. This separate, owner-only grant allows browsing host folders, installing Java and server software, and creating or importing servers on that host. It is not included in server permission presets. The creator receives full per-server access to each server they add, while existing shared servers remain available. Revoking the computer permission stops further creation without removing access to servers already added.

Accounts belong to the hosting panel. There is no public registration or universal MC Panel account. The local administrator entry identifies access to this computer and is not a remote account.
...
The remote account menu shows your email and panel address and provides **Sign out**. Each desktop connection has an isolated saved session. Quitting and reopening the app restores your connections and keeps you signed in while the host's session remains valid. **Sign out** ends your login. Expired sessions or reset account credentials require signing in again. An account with no allowed servers can remain signed in and will show servers when the owner grants access. Switching panels or signing out leaves Minecraft servers running on their host PCs. The host must keep MC Panel running with remote access enabled and reachable. Remote users use the same panel layout and pages as the desktop, with a responsive sidebar on phones. Available pages and actions follow their assigned permissions; connecting from the desktop does not grant local owner access. Update both the host and connecting desktop app to receive the latest interface.

### Desktop data

Both the installed and portable editions save the server registry and shared panel settings under the following directory. New servers use it by default; a custom installation folder keeps that server’s files, backups, and support data on the chosen drive:
...
Choose an actual build from the **Select build** placeholder, then give the server a name and choose its memory in GB. Under **Installation location**, keep the default or choose a dedicated empty folder on another drive. Choose **Browse** to select a drive and folder on the computer hosting MC Panel. The desktop app and web panel use the same folder navigator, with an optional new-folder name for new installations. While connected to another PC, the navigator shows folders on that PC. You can also enter an absolute path directly. Browsing does not create a folder until you confirm server creation. Custom installs keep Minecraft files in the selected folder and downloads, backups, and server settings beside it in `.mc-panel/<server-id>/`. Review the paths before creation. This choice applies to server software and modpacks, including their download staging. Shared Java runtimes installed with **Install Java** still use the app’s Java cache, whose path is shown in setup; selecting an existing compatible Java installation avoids that download. The **Java executable** dropdown shows one short entry per compatible major version, such as **JAVA 21**, using the newest verified installation for that major. Missing or incompatible installations are excluded. On supported Windows PCs, **Install Java** downloads the required Eclipse Temurin runtime when none is installed, verifies its published checksum and executable, and selects it automatically. These runtimes stay in MC Panel's private data folder and do not need administrator access or change your system PATH. Download progress and retry are available in setup; **Refresh Java** also finds runtimes installed elsewhere. The server port is available in **Advanced settings**. Setup checks Java again before you review the selected software or modpack, memory, and installation details. Accept the Minecraft EULA explicitly, then choose **Create and install**. The panel creates a separate server folder and shows installation progress. **Open Console** selects the new server; Minecraft stays stopped until you choose **Start**.
...
Choose **Import an existing server**, then browse to the folder containing its `server.properties`, startup files, and world. The desktop app and web panel use the same folder browser; you can also enter an absolute folder path. The path always refers to the computer hosting the selected panel. For another Windows PC, keep MC Panel running there and connect with the computer permission described above, or open its panel on that PC. To move the server to a different computer instead, first stop it and copy the complete server folder to the destination.
...
- **File Manager:** browse folders, create files and directories, edit text files up to 1 MB, and upload/download original files. The top breadcrumb follows the open folder; click any parent folder or **File Manager** to go back. Deletion moves files or whole folders to a protected, per-server **Recycle Bin** outside the Minecraft folder. Moving between drives copies and verifies the recovery data before removing the original, so large folders can take several minutes. The panel shows the move's progress; closing its dialog lets you continue browsing while the operation finishes. Restore returns an item to its original path and refuses to overwrite anything already there. Items remain until restored or explicitly deleted permanently; there is no automatic expiry. Row checkboxes support bulk moves, restores, and permanent deletion. Select-all affects visible rows without moving the checkbox, confirmations name selected recovery items, and failed actions stay selected for retry. Upload up to 20 files per request, 256 MB per file. Existing names are protected from accidental upload overwrites. Paths and symbolic links are checked to keep operations inside the configured server directory.
- **Backups:** create and download real `.tar.gz` archives with maximum gzip compression; configure interval, daily, or weekly schedules and retention. Each row shows the compressed size and, when recorded, the space saved. Existing archives are not recompressed. Files that are already compressed may shrink little further. Select individual archives or **Select all**, then use **Delete selected** to move them to Recycle Bin. Each row has **Download**, **Restore**, and **Delete** actions. To restore, stop the server and confirm replacement of its current files with that backup point; files created after the backup are removed. The archive remains available and the server stays stopped. Panel access and launch settings remain unchanged. Restore validates the archive and stages replacement on the server’s drive, with rollback if replacement fails; enough free space is required for staging. Remote users need the explicit **Restore backups** permission. Restoring a backup from Recycle Bin returns the archive to backup history without extracting it or changing server files. Failed moves remain selected for retry. Manual backups are kept until deleted. Retention moves excess scheduled backups to Recycle Bin after a new archive succeeds; recycled archives continue using disk space until explicitly deleted permanently. Symbolic links and Minecraft's temporary `session.lock` files are excluded.
- **Subusers:** create panel accounts and share private invitation links for phone, browser, or desktop access. New accounts include all current and future servers by default; exclude individual servers or choose **Only selected servers**. Choose permissions or use the Admin, Operator, or Viewer presets. Permission and server access changes apply to active sessions immediately; removing an account revokes its panel access. Existing users retain their selected servers and custom per-server permissions when upgrading. Direct access uses your public IP, a forwarded TCP port, and built-in HTTPS; no domain or email service is required. Recipients choose a password when accepting their invitation.
...
When Modrinth's batch lookup is unavailable, installed-file checks recover through individual file and project metadata lookups. Recovery shares a request budget across servers, caches verified identities, and stops repeated requests during a wider outage. Failed checks remain marked as unavailable; they never imply that a mod is up to date.
...
API clients can start an asynchronous backup with `POST /api/backups/jobs`, poll `GET /api/backups/jobs/:id`, and cancel it with `POST /api/backups/jobs/:id/cancel`. `GET /api/backups` includes the latest job. These endpoints use the same server selector as other APIs; reading progress requires `backup.read`, while starting and cancelling require `backup.create`. The original `POST /api/backups` endpoint still waits for completion and returns the completed archive for existing clients. `POST /api/backups/:id/restore` with `{ "confirm": true }` replaces the selected stopped server’s files from an existing backup and requires `backup.restore`.
...
## Invite someone to control your servers from their phone
...
5. Click **New user**, enter the recipient's email as their sign-in identifier, and choose permissions. New accounts use **All current and future servers**; uncheck individual servers to exclude them, or choose **Only selected servers**. Leave **Create invitation link** checked and click **Create subuser**. Copy the resulting link and send it privately through your usual text or email app. MC Panel sends no email. If clipboard access is unavailable, select and copy the displayed link manually; retrying **Copy link** keeps the same invitation.
...
Invitation links work once and expire after 24 hours; sessions last up to seven days. The raw invitation is displayed only when created, so copy it before closing its window. **Create invite link** produces a replacement and invalidates previous unused links for that account. **Reset access** creates a new password setup link for an activated panel account. After confirmation, its current password and sessions stop working immediately; the recipient regains access by accepting the new link. **Save permissions** changes what the person can do immediately. Unchecking a server revokes access to that server while keeping the rest of the account usable. Removing the account disables its sessions and invitation links across the panel. Accounts can also be saved before remote setup, then invited from their row later.

Existing per-server users keep their previous access when upgrading. The owner's Subusers page groups matching emails into a selected-server account with the existing per-server permission overrides. Grouping or editing these records does not combine passwords or grant additional sign-in access: each existing password and session still proves only its original servers, subject to the owner's current restrictions. Accepting an explicit panel invitation replaces those old credentials with one panel password and applies the account's selected server access. New servers remain excluded from migrated selected-server accounts unless the owner grants access or changes their mode to **All current and future servers**.
...
Only servers allowed by the signed-in account's current access settings appear in the remote session. An all-server account also gains access to future servers without another invitation or sign-in. Permission checks apply to every server request, including direct downloads. Panel account management, remote access settings, server removal, and owner configuration remain local administrative actions. The separate **Create and import servers** computer permission allows a trusted remote account to add servers on the host. Existing delegated per-server user management remains limited to that server and cannot edit panel-managed accounts or grant permissions the delegate does not have. Remote settings, hashed credentials, session state, and the private TLS certificate material are stored inside the panel data directory; protect that directory.
