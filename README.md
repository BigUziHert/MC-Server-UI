...
- **Audit Logs:** server starts, restarts, stops, commands, and settings appear under **Server**. File edits and mod additions, updates, and deletions appear under **Files**. **Players** contains operator, ban, kick, and whitelist actions; live player commands are recorded as requests sent to Minecraft. Backups and Subusers have their own categories. Legacy database activity is hidden. The most recent 2,000 events are retained per server.
